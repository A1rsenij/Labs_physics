...
data = pd.read_excel('2.2.3.xlsx', sheet_name='Лист3')
names = list(data)
...
for i in range(0, 16, 4):
    ...
    dQ = np.array(data[names[i + 1]])
    R = np.array(data[names[i + 2]])
    dR = np.array(data[names[i + 3]])
    k, cov = np.polyfit(Q, R, 1, cov=True)
    Rp = np.polyval(k, Q)
    plotline, caps, barlinecols = ax.errorbar(Q, R, xerr=dQ, yerr=dR, marker='.', label='$T = $' + str(T[i // 4]) + '$~^{\circ}C$', linestyle='none', color=colors[i // 4])
